...
//用于筛选
function getfilter($keywords,$fqwords,$page){
	//关键词
	$query=urlencode($keywords);
	//用红色字体高亮
	$pre = urlencode("<font color='red'>");
	...
	//获取内容
	$serializedResult = file_get_contents('http://118.24.29.39:8983/solr/snmeta/select?'.$fqwords.'&q='.$query.'&wt=phps&start='.$start.'&rows='.$rows.'&hl.fl=*&hl=true&hl.simple.post='.$post.'&hl.simple.pre='.$pre);
	//序列化PHP能处理的数据格式
	$result = unserialize($serializedResult);
	//取得返回数据结果,包括内容，总记录数和页数
	$response = $result['response'];
